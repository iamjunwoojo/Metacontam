...
class Louvain:
    ...
    def __init__(self, G, input_highpreval_taxid,verbose=False, randomized=False):
        # SETTINGS
...
########################
########################
    def pseudo_get_neighbour_communities(self, G, node, community_map, input_highpreval_taxid):
        neighbour_communities = defaultdict(int)
        for neighbour in input_highpreval_taxid:
            if (neighbour != node) and (neighbour in G[node]):
                neighbour_community = community_map[neighbour]
                w = G[node][neighbour].get("weight", 1)
                neighbour_communities[neighbour_community] += w
            elif (neighbour != node) and (neighbour not in G[node]):
                neighbour_community = community_map[neighbour]
                neighbour_communities[neighbour_community] += 0
        return neighbour_communities
    """ For example, given a pre-specified list [1,2,3,4,5],
    group these nodes first and treat the first node as the anchor node. """
    def pseudo_iterate(self):
        input_highpreval_taxid = self.input_highpreval_taxid
        first_input_highpreval_taxid = input_highpreval_taxid[0]
        if self.verbose:
            print("Pseudo iteration")
        # modified if we have made at least one change overall.
        # improved if we have made at least one change in the current pass.
        G = self.coarse_grain_graph
        self.tracker.initialize_network_statistics(G)
        ...
        if self.randomized:
            ...
            random.seed()
            random.shuffle(nodes)
        for node in self.input_highpreval_taxid:
            old_community = community_map[node]
            new_community = community_map[first_input_highpreval_taxid]
            if old_community == new_community:
                continue
            neighbour_communities = self.pseudo_get_neighbour_communities(
                G, node, community_map, input_highpreval_taxid
            )
            # Isolate the current node and find the best neighbouring
            # community (including checking the original).
            ...
            self.tracker.insert(node, new_community, new_incident_weight)
            if self.verbose:
                message = "Pseudo Moved node {} from community {} to community {}"
                print(message.format(node, old_community, new_community))
        self.relabel_community_map(community_map)
        self.community_history.append(community_map)
        self.coarse_grain_graph = self.generate_coarse_grain_graph(G, community_map)
...
def detect_communities(G , input_highpreval_taxid , verbose=False, randomized=False):
    ...
    """
    louvain = Louvain(G, input_highpreval_taxid, verbose=verbose, randomized=randomized)
    louvain.run()
